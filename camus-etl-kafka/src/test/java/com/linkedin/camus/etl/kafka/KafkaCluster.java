...
public class KafkaCluster {
	...
	public KafkaCluster(Properties baseProperties, int numOfBrokers)
			throws IOException {
	...
	private static class EmbeddedZookeeper {
		 ...
		 * 
		 * @throws IOException
		 *             if an error occurs during Zookeeper initialization.
		 */
		public EmbeddedZookeeper() throws IOException {
			this.port = getAvailablePort();
			this.snapshotDir = getTempDir();
			this.logDir = getTempDir();
			// this.factory = new NIOServerCnxn.Factory(new
			// InetSocketAddress("localhost", port), 1024);
			//
			// try {
			// int tickTime = 500;
			// factory.startup(new ZooKeeperServer(snapshotDir, logDir,
			// tickTime));
			// } catch (InterruptedException e) {
